...
export const vitalValidationOldSchema = Joi.object({
...
export const vitalUpdateValidationSchemaOld     = Joi.object({
...
const variableSchema = Joi.object({
    ...
    data_type: Joi.string().valid('number', 'string', 'boolean', 'file').required(),
    min: Joi.number(),
    max: Joi.number()
});
...
export const vitalTypeSchema = Joi.object({
    title: Joi.string().required(),
    icon: Joi.string().uri(),
    variables: Joi.array().items(variableSchema).required()
})
